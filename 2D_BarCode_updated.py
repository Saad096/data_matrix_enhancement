...
gray_blur = cv2.medianBlur(gray, 5)  # Only Odd Integer Number

# Apply thresholding to create a binary image
_, thresh = cv2.threshold(gray, 200, 300, cv2.THRESH_BINARY)
...
plt.title("Image")
plt.subplot(1, 2, 2)
plt.plot(np.array(x), np.array(y), 'ks')
...
def plot_squares(coordinates, side_length):
    for coord in coordinates:
        x, y = coord
        square = plt.Rectangle((x-side_length/2, y-side_length/2), side_length, side_length, edgecolor='black', facecolor='black')
        plt.gca().add_patch(square)


side_length = abs((x_dist[0]))
print(f'Side Length: {side_length}')

# Plotting squares around the coordinates
plt.scatter(x, y, color='blue', marker='s', label='Points')
